...
def page_generar():
    ...
    <div class="panel" style="margin-top:0;">
        ...
        <div class="small-muted" style="margin-top:8px;">El funcionario se registra para trazabilidad interna de quién toma el número.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("generar_documento_form", clear_on_submit=False):
        ...
        # Selector simplificado: solo se elige el funcionario que toma el número.
        # La oficina se obtiene automáticamente desde el catálogo de funcionarios,
        # evitando problemas de actualización entre oficina y funcionario.
        funcionarios_activos = funcionarios.copy().fillna("")
        funcionarios_activos["etiqueta_funcionario"] = funcionarios_activos.apply(
            lambda r: f"{str(r['funcionario']).strip()}" + (f" — {str(r['oficina']).strip()}" if str(r['oficina']).strip() else ""),
            axis=1,
        )
        funcionario_label = st.selectbox(
            "Funcionario que toma el número",
            funcionarios_activos["etiqueta_funcionario"].tolist(),
            key="funcionario_toma_numero_simple",
        )
        funcionario_row = funcionarios_activos[funcionarios_activos["etiqueta_funcionario"] == funcionario_label].iloc[0]
        funcionario = str(funcionario_row["funcionario"]).strip()
        oficina = str(funcionario_row.get("oficina", "")).strip() or "Sección Gestión Ambiental"

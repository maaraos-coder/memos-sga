...
.logo-wrap img {{max-width: 460px !important; height:auto !important; display:block; margin:auto;}}
.main-title {{
    font-size: 2.25rem;
    line-height: 1.15;
    font-weight: 850;
...
def mostrar_inicio():
    ...
    col_logo, col_title = st.columns([2.2, 3.0], vertical_alignment="center")
    with col_logo:
        if LOGO_PATH.exists():
...
def page_correlativos():
    ...
    st.warning("Use esta opción solo para migrar desde el sistema antiguo o corregir el último número utilizado. El próximo documento se generará sumando 1 al número guardado.")
    ...
    with st.form("correlativos_form"):
        memo = st.number_input("Último número utilizado de Memorándum", min_value=0, value=int(actual.get("Memorándum", 0)), step=1)
        ordn = st.number_input("Último número utilizado de Ordinario", min_value=0, value=int(actual.get("Ordinario", 0)), step=1)
    ...
    conn = connect()
    aud = pd.read_sql_query("SELECT fecha_hora, usuario, accion, detalle FROM auditoria ORDER BY id DESC LIMIT 30", conn)
    conn.close()
        ...
        if not usuario.strip() or not nombre.strip() or not temp:
            ...
            if not valido:
                ...
                if existe:
                    ...
                    conn.commit(); conn.close()
                    st.success(f"Usuario creado. Entréguele estas credenciales: usuario {usuario.strip()} y la contraseña temporal definida. Al ingresar deberá cambiarla.")
                    st.rerun()
...
def page_admin():
    ...
    tab1, tab2, tab3 = st.tabs(["Correlativos", "Usuarios", "Anular documento"])
    with tab1:
        page_correlativos()
    with tab2:

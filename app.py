...
/* =======================
   BOTONES AZULES INSTITUCIONALES
   ======================= */
...
button[kind="secondary"] {{
    ...
    box-shadow: 0 6px 16px rgba(0,94,184,.20) !important;
    transition: all .20s ease !important;
}}

/* Fuerza el texto blanco en todas las versiones de Streamlit */
div.stButton > button *,
div.stFormSubmitButton > button *,
button[kind="primary"] *,
button[kind="secondary"] * {{
...
button[kind="secondary"]:hover {{
    ...
    box-shadow: 0 8px 20px rgba(0,94,184,.28) !important;
}}
...
/* Sidebar: botón cerrar sesión ancho completo */
...
def tipo_key(tipo: str) -> str:
    ...
    return mapa.get(tipo, str(tipo).upper().replace(" ", "_"))
...
def tipo_display(key: str) -> str:
    ...
    return mapa.get(str(key).upper(), str(key))
...
def init_db():
    ...
    with engine.begin() as conn:
        ...
        # Desde esta versión, los documentos anulados conservan su registro histórico,
        # pero no bloquean la reutilización de un número que haya sido liberado
        # explícitamente desde la aplicación. Los anulados antiguos NO se reciclan.
        ...
        conn.execute(text("""
            ...
            where coalesce(estado, 'Emitido') not like 'Anulado%'
        """))

        conn.execute(text("""
            create table if not exists numeros_liberados (
              tipo text not null,
              anio integer not null,
              numero integer not null,
              documento_anulado_id bigint,
              liberado_por text,
...
def asegurar_correlativos():
    """Crea los correlativos del año si no existen.

    Importante: no fuerza el correlativo al máximo de documentos existentes.
    Esto permite que el administrador pueda reiniciar manualmente a 0
    después de borrar documentos de prueba.
    """
    anio = ahora_chile().year
    engine = get_engine()
    with engine.begin() as conn:
        for tipo in ["Memorándum", "Ordinario", "Informe Técnico"]:
            key = tipo_key(tipo)
            conn.execute(text("""
...
def ultimos_documentos():
    df = db_query_df("""
        ...
        from documentos
        where tipo in ('Memorándum','Ordinario','Informe Técnico')
...
def generar_documento(tipo, oficina, funcionario, dest_rows, materia, usuario_login):
    ...
    if isinstance(dest_rows, pd.DataFrame):
        destinos_df = dest_rows.copy().fillna("")
    else:
        destinos_df = pd.DataFrame([dict(dest_rows)]).fillna("")
    ...
    for _, r in destinos_df.iterrows():
        ...
        if jefatura:
            jefaturas.append(jefatura)
        if nombre:
            nombres.append(nombre)

        etiqueta = f"{jefatura} — {seccion}" if jefatura or seccion else nombre
        if nombre and (jefatura or seccion):
            etiqueta += f" — {nombre}"
        if etiqueta.strip():
            partes_destinatario.append(etiqueta.strip())

    destinatario = " | ".join(partes_destinatario)
    seccion_join = " | ".join(secciones)
    jefatura_join = " | ".join(jefaturas)
    nombres_join = " | ".join(nombres)
    ...
    with engine.begin() as conn:
        ...
        row = conn.execute(
            text("select ultimo_numero from correlativos where tipo=:key and anio=:anio for update"),
            {"key": key, "anio": anio}
        ).mappings().fetchone()

        # Primero se reutiliza SOLO un número liberado expresamente desde esta
        # versión. No se buscan documentos anulados históricos.
        liberado = conn.execute(text("""
            select numero
            from numeros_liberados
            where tipo=:key and anio=:anio
            order by numero asc
            for update skip locked
            limit 1
        """), {"key": key, "anio": anio}).mappings().fetchone()

        if liberado:
            nuevo = int(liberado["numero"])
        else:
            # El correlativo nunca debe quedar por debajo del mayor número vigente.
            max_vigente = conn.execute(text("""
                select coalesce(max(numero), 0) as max_numero
                from documentos
                where tipo=:tipo and anio=:anio
                  and coalesce(estado, 'Emitido') not like 'Anulado%'
            """), {"tipo": tipo, "anio": anio}).mappings().fetchone()
            base = max(int(row["ultimo_numero"] or 0), int(max_vigente["max_numero"] or 0))
            nuevo = base + 1
        ...
        """), {
            ...
            "seccion": seccion_join,
            "jefatura": jefatura_join,
            "nombre": nombres_join,
        ...
        if liberado:
            conn.execute(text("""
                delete from numeros_liberados
                where tipo=:key and anio=:anio and numero=:numero
            """), {"key": key, "anio": anio, "numero": nuevo})
        else:
            conn.execute(text("""
...
def mostrar_inicio():
    ...
    with col_title:
        ...
        st.markdown('<div class="sub-title">Memorándums · Ordinarios · Informes Técnicos<br>Sección de Gestión Ambiental · Departamento de Acción Sanitaria</div>', unsafe_allow_html=True)
    ...
    with c3:
        st.markdown(f"""
        <div class="card card-blue">
            <div class="card-label">Último Informe Técnico emitido</div>
            <div class="big-number-blue">{inf_num}</div>
            <div>Emitido por: <b>{inf_func}</b></div>
            <div class="small-muted" style="margin-top:8px;">{inf_fecha}</div>
        </div>
        """, unsafe_allow_html=True)
...
def page_generar():
    ...
    <div class="panel" style="margin-top:0;">
        ...
        <div class="small-muted" style="margin-top:8px;">El funcionario se registra automáticamente según el usuario que inició sesión. Puede seleccionar uno o varios destinatarios.</div>
    </div>
    """, unsafe_allow_html=True)

    user_actual = st.session_state.get("user", {})
    nombre_usuario = str(user_actual.get("nombre", "")).strip()
    oficina_usuario = str(user_actual.get("oficina", "")).strip()

    funcionarios_activos = funcionarios.copy().fillna("")
    match = funcionarios_activos[
        funcionarios_activos["funcionario"].astype(str).str.strip().str.lower().eq(nombre_usuario.lower())
    ]

    if not match.empty:
        funcionario = str(match.iloc[0]["funcionario"]).strip()
        oficina = str(match.iloc[0].get("oficina", "")).strip() or oficina_usuario or "Sección Gestión Ambiental"
    else:
        funcionario = nombre_usuario or str(user_actual.get("usuario", "")).strip()
        oficina = oficina_usuario or "Sección Gestión Ambiental"

    with st.form("generar_documento_form", clear_on_submit=True):
        tipo = st.radio("Tipo de documento", ["Memorándum", "Ordinario", "Informe Técnico"], horizontal=True)

        st.selectbox(
            "Funcionario que toma el número",
            [funcionario],
            index=0,
            key="funcionario_toma_numero_usuario",
            disabled=True,
        )
        ...
        dest_label_to_idx = {label: i for i, label in enumerate(dest_labels)}

        dest_labels_sel = st.multiselect(
            "Destinatario(s)",
            options=dest_labels,
            default=[],
            placeholder="Seleccione uno o más destinatarios...",
            key="destinatarios_generar_multiselect",
            help="Puede seleccionar más de un destinatario para el mismo número de documento."
        )

        materia = st.text_area("Materia", placeholder="Ej.: Remite antecedentes técnicos...", height=120, key="materia_generar_text")
        generar = st.form_submit_button("Generar número", type="primary")

    if generar:
        if not dest_labels_sel:
            st.warning("Debe seleccionar al menos un destinatario.")
        elif not materia.strip():
            st.warning("Debe ingresar la materia del documento.")
        else:
            dest_indices = [dest_label_to_idx[label] for label in dest_labels_sel]
            dest_rows = destinatarios.iloc[dest_indices].copy()
            n, anio, fecha = generar_documento(tipo, oficina, funcionario, dest_rows, materia, st.session_state["user"]["usuario"])

            if len(dest_labels_sel) == 1:
                dest_resumen = dest_labels_sel[0]
            else:
                dest_resumen = f"{len(dest_labels_sel)} destinatarios seleccionados"

            st.markdown(f"""
            <div class="result-card">
                <div class="result-ok">Número generado correctamente</div>
                <div class="result-number">{tipo} N° {n:03d}/{anio}</div>
                <div class="result-meta">Funcionario que toma el número: {funcionario}</div>
                <div class="result-meta">Destinatario(s): {dest_resumen}</div>
                <div class="result-time">Fecha y hora de Santiago de Chile: {fecha} hrs.</div>
            </div>
            """, unsafe_allow_html=True)
            st.success("Formulario listo para generar otro documento.")
...
def page_cambiar_password(forzado=False):
    ...
    with st.form("cambiar_password_form"):
        ...
        ok = st.form_submit_button("Guardar nueva contraseña", type="primary")
    if ok:
...
def page_correlativos():
    ...
    inf_actual = int(actual.get("INFORME_TECNICO", 0))
    c1, c2, c3 = st.columns(3)
    ...
    c3.metric("Último Informe Técnico registrado", f"N° {inf_actual:03d}/{int(anio)}", f"Próximo: N° {inf_actual + 1:03d}/{int(anio)}")
    with st.form("correlativos_form"):
        ...
        inf = c3.number_input("Último número utilizado de Informe Técnico", min_value=0, value=inf_actual, step=1)
        ok = st.form_submit_button("Guardar correlativos", type="primary")
    if ok:
        engine = get_engine()
        with engine.begin() as conn:
            for key, valor, ant, label in [("MEMORANDUM", int(memo), memo_actual, "Memorándum"), ("ORDINARIO", int(ordn), ord_actual, "Ordinario"), ("INFORME_TECNICO", int(inf), inf_actual, "Informe Técnico")]:
                conn.execute(text("""
                    insert into correlativos(tipo, anio, ultimo_numero, actualizado_por, actualizado_en)
...
def page_borrar_pruebas_admin():
    st.subheader("Borrar documentos de prueba / reiniciar numeración")
    st.warning("Esta acción es irreversible. Úsela solo para limpiar pruebas antes de iniciar el uso oficial del sistema.")

    anio = st.number_input("Año a limpiar", min_value=2024, max_value=2100, value=ahora_chile().year, step=1, key="anio_limpieza_pruebas")

    resumen = db_query_df("""
        select tipo, count(*) as cantidad, coalesce(max(numero), 0) as ultimo
        from documentos
        where anio=:anio
        group by tipo
        order by tipo
    ...
    if resumen.empty:
        st.info(f"No existen documentos registrados para el año {int(anio)}.")
    else:
        st.dataframe(resumen, use_container_width=True, hide_index=True)

    st.markdown("---")
    st.markdown("**Opción recomendada para partir desde cero:** borrar documentos del año seleccionado y reiniciar todos los correlativos en 0.")
    confirmar = st.text_input("Para confirmar escriba BORRAR", key="confirmar_borrado_pruebas")
    incluir_auditoria = st.checkbox("Registrar esta acción en auditoría", value=True)

    if st.button("Borrar documentos del año y reiniciar correlativos", type="primary"):
        if confirmar.strip().upper() != "BORRAR":
            st.error("Debe escribir BORRAR para confirmar la acción.")
            return
        engine = get_engine()
        with engine.begin() as conn:
            borrados = conn.execute(text("delete from documentos where anio=:anio"), {"anio": int(anio)}).rowcount
            for key in ["MEMORANDUM", "ORDINARIO", "INFORME_TECNICO"]:
                conn.execute(text("""
                    insert into correlativos(tipo, anio, ultimo_numero, actualizado_por, actualizado_en)
                    values(:key, :anio, 0, :u, now())
                    on conflict(tipo, anio) do update
                    set ultimo_numero=0, actualizado_por=:u, actualizado_en=now()
                """), {"key": key, "anio": int(anio), "u": st.session_state["user"]["usuario"]})
            if incluir_auditoria:
                log_auditoria_db(conn, st.session_state["user"]["usuario"], "Borra pruebas y reinicia correlativos", f"Año {int(anio)}. Documentos borrados: {borrados}. Correlativos en 0.")
        registros_df.clear()
        ultimos_documentos.clear()
        st.cache_data.clear()
        st.success(f"Se borraron {borrados} documentos del año {int(anio)} y los correlativos quedaron en 0.")
        st.rerun()

    st.markdown("---")
    st.markdown("**Solo reiniciar correlativos en 0**")
    st.caption("Use esta opción solo si no existen documentos registrados para ese año. Si existen documentos, podría intentar repetir números ya usados.")
    confirmar_reset = st.text_input("Para solo reiniciar escriba REINICIAR", key="confirmar_reset_correlativos")
    if st.button("Solo reiniciar correlativos en 0"):
        if confirmar_reset.strip().upper() != "REINICIAR":
            st.error("Debe escribir REINICIAR para confirmar.")
            return
        total_docs = db_query_one("select count(*) as n from documentos where anio=:anio", {"anio": int(anio)})["n"]
        if int(total_docs) > 0:
            st.error("No se puede reiniciar solo el correlativo porque existen documentos registrados para ese año. Primero bórrelos o anúlelos según corresponda.")
            return
        engine = get_engine()
        with engine.begin() as conn:
            for key in ["MEMORANDUM", "ORDINARIO", "INFORME_TECNICO"]:
                conn.execute(text("""
                    ...
                    set ultimo_numero=0, actualizado_por=:u, actualizado_en=now()
                """), {"key": key, "anio": int(anio), "u": st.session_state["user"]["usuario"]})
            log_auditoria_db(conn, st.session_state["user"]["usuario"], "Reinicia correlativos", f"Año {int(anio)}. Correlativos en 0.")
        registros_df.clear()
        ultimos_documentos.clear()
        st.cache_data.clear()
        st.success("Correlativos reiniciados en 0.")
        st.rerun()
...
def page_admin():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Correlativos", "Usuarios", "Catálogos", "Anular documento", "Reinicio / pruebas", "Auditoría"])
    with tab1:
        page_correlativos()
    with tab2:
    ...
    with tab4:
        ...
        if not df.empty:
            ...
            st.info("Al anular, el registro se conserva para trazabilidad y ESTE número queda disponible para reutilización. Los números anulados antes de esta versión no se reutilizan automáticamente.")
            if st.button("Anular registro"):
                row = df[df["etiqueta"] == etiqueta].iloc[0]
                estado_actual = str(row.get("estado", "Emitido"))
                if estado_actual.startswith("Anulado"):
                    st.warning("Este documento ya está anulado.")
                else:
                    ...
                    with engine.begin() as conn:
                        ...
                        conn.execute(text("""
                            insert into numeros_liberados(tipo, anio, numero, documento_anulado_id, liberado_por, liberado_en)
                            values(:tipo, :anio, :numero, :doc_id, :usuario, now())
                            on conflict(tipo, anio, numero) do nothing
                        """), {
                            "tipo": tipo_key(str(row["tipo_documento"])),
                            "anio": int(row["anio"]),
                        ...
                        log_auditoria_db(
                            ...
                            "Anula y libera número",
                            etiqueta
                        )
                    registros_df.clear()
                    ultimos_documentos.clear()
                    st.success(f"Registro anulado. El N° {int(row['numero']):03d}/{int(row['anio'])} quedó disponible para reutilización.")
                    st.rerun()
        else:

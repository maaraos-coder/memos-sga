...
def page_catalogos_admin():
    ...
    st.markdown("""
    <div class="admin-help">
        Estos catálogos alimentan los desplegables de la aplicación. Por ahora se actualizan editando los CSV en GitHub.
        Más adelante se puede agregar edición directa desde esta pantalla.
    </div>
    """, unsafe_allow_html=True)
    t1, t2 = st.tabs(["Funcionarios por oficina", "Destinatarios"])
    with t1:
        try:
            st.dataframe(read_funcionarios(), use_container_width=True, hide_index=True)
        except Exception as e:
            st.error(f"No se pudo leer funcionarios.csv: {e}")
    with t2:
        try:
            st.dataframe(read_destinatarios(), use_container_width=True, hide_index=True)
        except Exception as e:
            st.error(f"No se pudo leer destinatarios.csv: {e}")

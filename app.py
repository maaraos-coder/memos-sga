...
def page_generar():
    ...
    with st.form("generar_documento_form", clear_on_submit=False):
        ...
        c1, c2 = st.columns(2)
        with c1:
            oficinas = sorted(funcionarios["oficina"].dropna().astype(str).unique().tolist())
            oficina = st.selectbox(
                "Oficina de la Sección Gestión Ambiental que solicita/toma el número",
                oficinas,
                key="oficina_origen_select"
            )

        funcionarios_filtrados = funcionarios[
            funcionarios["oficina"].astype(str).eq(str(oficina))
        ].copy()

        lista_funcionarios = funcionarios_filtrados["funcionario"].dropna().astype(str).tolist()

        # Cuando cambia la oficina, se limpia la selección anterior para que no quede
        # un funcionario perteneciente a otra oficina.
        if st.session_state.get("_oficina_anterior") != oficina:
            st.session_state["_oficina_anterior"] = oficina
            st.session_state.pop("funcionario_origen_select", None)
        ...
        with c2:
            if lista_funcionarios:
                funcionario = st.selectbox(
                    ...
                    lista_funcionarios,
                    key="funcionario_origen_select"
                )
            else:
                funcionario = ""
                st.warning("No hay funcionarios activos asociados a esta oficina.")
    ...
    if generar:
        if not funcionario:
            st.warning("Debe seleccionar un funcionario válido para la oficina seleccionada.")
...
def page_admin():
    ...
    c1, c2, c3 = st.columns(3)
    c1.markdown(f'<div class="admin-card"><div class="admin-card-title">Documentos registrados</div><div class="admin-card-text"><b>{len(df)}</b> documentos en el sistema.</div></div>', unsafe_allow_html=True)
...
def main():
    ...
    st.caption("Versión 2.0 Supabase · SEREMI de Salud Región Metropolitana · Departamento de Acción Sanitaria · Sección Gestión Ambiental")
